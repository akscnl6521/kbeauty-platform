 ...
 *
 * 라벨의 **닫는 괄호**가 첫 성분에 붙어 오기도 한다 — `] 정제수` (2026-08-09
 * 클라뷰 실측, 세 제품이 전부 이것 하나에 막혀 있었다).
 *
 * **성분명 안에 쓰이는 기호는 건드리지 않는다** — 맨 앞에 있고 **공백을 사이에 두고**
 * 글자가 이어질 때만 뗀다. `1,2-헥산다이올` 의 하이픈처럼 이름 일부인 기호는 그대로 둔다.
 */
