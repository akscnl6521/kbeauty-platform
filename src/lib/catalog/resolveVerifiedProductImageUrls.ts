 ...
 *
 * Requests are queried in bounded chunks so recommendation candidates beyond
 * the first 80 products can still receive their verified image.
 */
export async function resolveVerifiedProductImageUrls(
  ...
  for (let index = 0; index < ids.length; index += QUERY_CHUNK_SIZE) {
    const chunk = ids.slice(index, index + QUERY_CHUNK_SIZE);
