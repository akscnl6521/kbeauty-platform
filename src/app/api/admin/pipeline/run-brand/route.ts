...
/**
 * POST /api/admin/pipeline/run-brand
 * body: { brandLimit?: 1, mode?, productLimitPerBrand? }
 * Seeds from catalog (limit 1 by default) — no hardcoded brand name required.
 */
export const POST = withAdminAuth(async (request: NextRequest, _ctx, session) => {
  try {
    ...
    const batch = await createPipelineBatch({
      ...
      brandLimit: Number(body.brandLimit ?? 1),
