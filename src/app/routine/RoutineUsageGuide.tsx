...
function parseGuide(value: unknown, productId: string): StoredUsageGuide | null {
  ...
  if (!["morning", "evening", "weekly", "as_needed"].includes(String(row.frequency))) return null;
  ...
  if (typeof row.verifiedAt !== "string" || Number.isNaN(new Date(row.verifiedAt).getTime())) return null;
  ...
  if (row.media && typeof row.media === "object" && !Array.isArray(row.media)) {
    ...
    if (
      ...
      media = {
        ...
        disclosureText:
          typeof candidate.disclosureText === "string" ? candidate.disclosureText.trim() || null : null,
      };
    }
  }
...
export default function RoutineUsageGuide({ productId, locale }: { productId: string; locale: Locale }) {
  ...
  useEffect(() => {
    try {
      ...
      setGuide(values.map((value) => parseGuide(value, productId)).find(Boolean) ?? null);
    } catch {
      setGuide(null);
    }
  }, [productId]);
  ...
  if (!guide) {
    return <p className="mt-3 border-t border-pink-100 pt-3 text-xs text-gray-500">{copy.empty}</p>;
  }

  const timing = copy[guide.frequency];
  const verified = new Intl.DateTimeFormat(locale === "ko" ? "ko-KR" : locale === "ja" ? "ja-JP" : "en-US").format(
    new Date(guide.verifiedAt),
  );

  return (
    <div className="mt-3 border-t border-pink-100 pt-3 text-xs text-gray-700">
      <p className="font-semibold text-gray-900">{copy.title}</p>
      <dl className="mt-2 space-y-1.5">
        <div><dt className="inline font-semibold">{copy.amount}: </dt><dd className="inline">{guide.amountLabel}</dd></div>
        <div><dt className="inline font-semibold">{copy.timing}: </dt><dd className="inline">{timing}</dd></div>
        <div><dt className="inline font-semibold">{copy.area}: </dt><dd className="inline">{guide.applicationArea.join(", ")}</dd></div>
      </dl>
      <div className="mt-2">
        <p className="font-semibold">{copy.method}</p>
        <ol className="mt-1 list-decimal space-y-1 pl-4">
          {guide.methodSteps.map((step) => <li key={step}>{step}</li>)}
        </ol>
      </div>
      {guide.cautionText.length > 0 && (
        <div className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-amber-900">
          ...
          <ul className="mt-1 list-disc space-y-1 pl-4">
            {guide.cautionText.map((item) => <li key={item}>{item}</li>)}
          </ul>
        </div>
      )}
      {guide.media && (
        <div className="mt-2">
          <a href={guide.media.sourceUrl} target="_blank" rel="noreferrer" className="font-semibold text-[#C2185B] underline hover:no-underline">
            {copy.source}
          </a>
          {guide.media.disclosureText && <p className="mt-1 text-[11px] text-gray-500">{guide.media.disclosureText}</p>}
        </div>
      )}
      <p className="mt-2 text-[11px] text-gray-500">{copy.verified}: {verified}</p>
    </div>
  );
}
