...
const DECISIONS = [
  ...
  { value: "revoked", label: "권리 철회" },
] as const;

type Decision = (typeof DECISIONS)[number]["value"];

/**
 * Records a review decision. Approval is refused by the server when the asset is
 * not actually publishable, so the panel surfaces that reason rather than
 * pretending the click succeeded.
 */
export function MediaReviewDecisionPanel({
  assetId,
  canApprove,
}: {
  assetId: string;
  ...
  async function submit(event: React.FormEvent) {
    ...
    setMessage(null);
    try {
      const res = await fetch(`/api/admin/media-review/${assetId}`, {
        method: "POST",
