...
성분 사전 1,341 → **1,494행**, 별칭 0 → 48행. 제품 29건 활성화.

**막고 있던 것들을 순서대로**

1. 국내몰이 전성분을 텍스트로 안 준다 → 탐침에 그 조건을 넣어 몰을 고름
2. 옛 매처가 하이픈을 공백으로 바꿔 놓아 식약처와 안 맞음 → 구분자만 무시하는
   느슨한 키를 마지막 수단으로 (모호한 166종은 쓰지 않는다)
3. 활성화 검사기가 `ingredient_aliases` 를 안 봄 → 실제 매처보다 엄격했다
4. **`--production` 을 npm 이 자기 플래그로 먹음** → Staging 에 쓰고 있었다.
   `--target-production` 으로 바꾸고 대상 DB 를 첫 줄에 찍는다.

**등록 품질** — `[주주우대]` 9,900원 같은 조건부 가격, 비비크림이 toner 로 들어오던
유형 오분류, 브랜드 라인이 이름을 공유해 서로 다른 제품이 묶이던 중복 판정을 고쳤다.

캐시 버전 V3 → V4.

---

## 2026-08-08 국내몰 3곳 확대 + 등록 품질 필터 (활성화는 권한 대기)

- 국내몰 후보 38곳을 실제로 열어 확인 → 달바·티르티르·설화수 추가 (재고 302건)
- 제품 37건 등록 (231~267). **전부 비활성** — 성분 사전 확장 권한이 막혀 대기.
- 등록 품질 필터 3종 + 회귀 테스트 (CI 포함):
  조건부 가격·단종(`[주주우대]` 9,900원 등) · 얼굴 스킨케어가 아닌 것(비비크림이
  toner 로 들어왔다) · 같은 몰 안 중복(포장·수량만 다를 때로 좁힘)
- 탐침 표본을 사이트맵 앞쪽에서 자르지 않고 **고르게 퍼뜨려** 뽑도록 수정.
  앞 6건만 보면 판정이 뒤집힌다 — 달바가 그 때문에 하루 전 탈락했었다.
- **08-07 의 «식약처에 그 성분이 없다» 는 결론을 정정한다.** 급조한 XML 파서가
  빈 값을 돌려준 것이었고, 실제로는 미매칭 229종 중 148종이 식약처에 있다.

---

## 2026-08-08 🚀 배포 — 추천 풀 36 → 51건이 라이브에 반영

PR #43 병합 → Vercel 자동 배포. `main` `9039f74` → `51ce8ba`.
라이브 `/api/health` 로 반영 확인, 주요 화면 6종 HTTP 200.

**사용자에게 닿은 변화**

- 추천 가능한 제품이 **36 → 51건**. 시나리오 30개 전부 최소 3개 이상 뜨고,
  국내 구매처가 하나도 없는 시나리오는 0개다.
- 브랜드 11 → 12개.
- 캐시 버전 V3 — 예전에 결과를 본 사람도 새 Top 5 를 다시 계산해서 본다.

**무엇이 막고 있었나** — 성분이 없어서가 아니라 **한글 이름이 하나가 아니어서**였다.
자세한 것은 아래 2026-08-07 항목.

---

## 2026-08-07 한글 이명(異名)을 못 읽어 막혀 있던 국내 제품 15건 활성화 — 추천 풀 36 → 51

국내몰에서 등록한 제품이 활성화 게이트의 `unmatchedIngredientCount === 0` 에서
줄줄이 막혀 있었다. 원인은 **성분이 없는 게 아니라 이름이 하나가 아니었던 것**이다.

| 국내몰 표기 | 우리 사전 | 같은 성분인가 |
|---|---|---|
| `살리실릭애씨드` | `살리실산` | 예 — Salicylic Acid |
| `하이알루로닉애씨드` | `히알루론산` | 예 — Hyaluronic Acid |
| `달팽이점액여과물` | `달팽이 분비물 여과물` | 예 — Snail Secretion Filtrate |
| `1,2-헥산다이올` | `2 헥산다이올` | 예 — 사전 쪽이 쉼표에 잘려 있었다 |

### 고친 것

1. **식약처 이명 사전을 자동 생성한다** — `scripts/build-mfds-korean-synonyms.ts`.
   식약처 «화장품 원료성분정보» 21,863행에서 **영문명이 정확히 일치**하는 것만
   우리 사전 행에 붙인다. 27종 / 33개. 우리가 만든 표기는 하나도 없다.
   결과는 `src/lib/pipeline/mfdsKoreanSynonyms.generated.ts` (자동 생성, 손대지 않는다).
   덮어쓰지 않고 **더한다** — 화면에 나가는 `name_ko` 는 그대로 둔다.
2. **한글 규제 문구를 성분표에서 걷어낸다** — `주름개선` · `｢화장품법｣에 따른` ·
   `사용 시의` · `자외선 차단제품 등) 의 경우` · `해당없음`. 국내몰은 기능성 표시와
   법령 인용을 전성분 뒤에 붙여 판다.
3. **제형 라벨 `[블루드롭]` 에서 쪼갠다** — `(1번)` 과 같은 이유로 **끊지 않고 쪼갠다.**
   뒤쪽 목록을 버리면 그쪽 알레르겐이 사라진다.
4. 잘라낸 자리에 여는 괄호만 남던 것 수정 — `아데노신 ｢화장품법｣…` → `아데노신 ｢`.
5. 재분리 스크립트의 «토큰이 줄면 건너뛴다» 안전장치를 **없애지 않고 좁혔다** —
   사라지는 조각 하나하나가 안내 문구임을 따로 확인했을 때만 허용한다.
   스플리터를 믿고 통과시키면, 스플리터가 틀린 날 성분이 조용히 지워진다.
6. `KEY_ACTIVE_DICTIONARY` 22항목에 한글 키 추가 — 한글 성분표에서 주요 성분이
   하나도 안 뽑혀 등록 후보 15건이 통째로 버려지고 있었다.
...
활성화된 15건 중 4건(195·200·207·214)은 **전부터 같은 이유로 막혀 있던 기존 제품**이다.
즉 이 결함은 신규 등록만의 문제가 아니었다.

### 아직 막힌 것

227~230(Klairs 메이플·블루드롭)은 `설탕단풍수액` · `꽃단풍나무껍질추출물` ·
`리마콩씨추출물` 이 우리 사전 1,341행에 **실제로 없다.** 이명 문제가 아니라
사전 확장 문제이고, 식약처 적재가 slug 충돌로 0행에서 멈춘 건과 같은 뿌리다.
지어내지 않고 남겨 둔다.

---

## 2026-08-04 🚀 배포 — 7/30~8/4 작업 20커밋이 라이브에 반영

PR #37 병합 → Vercel 자동 배포. `main` `e5505c1` → `6659e14`.
라이브 `/api/health` 버전 확인, 주요 화면 6종 HTTP 200.

**사용자에게 닿은 변화**

- 국내 구매 링크가 **뜬다** (배포 전에는 오퍼가 전부 US 라 안 떴다) — 13개 제품
- 브랜드 쏠림 해소 — 「건성+장벽」 Top 5 전부 COSRX 이던 것이 시나리오당 3~4개 브랜드
- 오염된 전성분을 가진 제품은 활성화 게이트가 막는다
- 알레르겐 검출 향료 16/18 → 18/18, 추천 풀 17건 전부 전성분 보유

**배포 직후 실측** — 시나리오 30개 중 최소 개수 미달 0 · 국내 구매처 없는 시나리오 0 ·
브랜드 상한 초과 0 · 추천 풀 전성분 검증 17/17.

**판단으로 남긴 것** — 카탈로그 확대는 현 방식으로 막혔고(§39·§42),
규제 성분 판정은 식약처 데이터로 안 된다(§43). 둘 다 실패를 근거와 함께 기록했다.
...
- 전성분 4건 보강 (id 4 · 28 · 76 · 135). id 135 는 접두 일치(`niacin` ⊂
  `niacinamide`)를 넣은 뒤에야 대조가 붙었다.
