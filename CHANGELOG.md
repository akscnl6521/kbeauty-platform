...
- 터치 기기는 호버 없이 탭 선택만 사용 · 선택 시 정보 카드로 스크롤
- SVG 히트 영역·칩 터치 타깃 보강 · 안내 문구 탭 기준
- Preview READY (`7893766`) · Production 미배포
...
- 피부 문진(`/quiz`) + DomainQuizClient 공통 적용
- Production 미배포

### 헤더/푸터 네비 보강

- 헤더: 피부 문진 · 추천 결과 추가 (중복「내 피부 관리」는 로그인 시에만)
- 푸터: 주요 바로가기와 법적 링크 분리
- Production 미배포
...
- 이전 분석 있으면 결과·루틴 바로가기 · 하단 CTA도 상태별 분기
- Production 미배포

### 결과 탐색 브랜드 칩 필터

- `/results`에 브랜드 칩 추가 · 제형 칩과 교차 필터
- Production 미배포
