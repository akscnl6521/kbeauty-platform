...
function preprocess(raw: string): string {
  ...
  // 1. «사용상의» 는 뒤에 무엇이 오든(또는 아무것도 안 와도) 성분 목록의 끝이다.
  //    성분명에 «사용상의» 가 들어가는 경우는 없다.
  const i = s.search(/사용상의|사용할\s*때의/);
  if (i > -1) s = s.slice(0, i);

  // 2. `!---!` 같은 마커. 하이픈이 정규화 단계에서 공백이 되기 전에 없앤다.
  s = s.replace(/!\s*-{2,}\s*!/g, " ").replace(/!{2,}/g, " ");

  // 3. «(1번)» «(3번)» 구획 라벨. 공백으로 바꾸면 라벨 앞뒤 성분이 한 토큰으로
  //    붙어 버린다(`로즈마리잎오일 (3번) 정제수` -> `로즈마리잎오일 정제수`).
  //    라벨은 실제로 구획의 경계이므로 쉼표로 바꿔 끊는다.
  s = s.replace(/\(\s*\d+\s*번\s*\)/g, ", ");
