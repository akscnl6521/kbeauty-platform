#!/usr/bin/env node
/** Staging-only: apply Evidence phase-3 concern expansion. Never touches Production. */
...
function q(sql) {
  const f = path.join(tmpdir(), `kb-p3-${process.pid}.sql`);
...
const src = path.join(
  root,
  "supabase/migrations/20260714092000_seed_evidence_layer_phase3_concerns.sql"
);
const dst = path.join(tmpdir(), `kb-p3-apply-${process.pid}.sql`);
...
const concerns = q(
  `SELECT code FROM skin_concerns WHERE review_status='approved' AND code IN ('acne','pigmentation','antiaging','pores','uv','redness','dryness','sensitivity') ORDER BY code;`
);
const count = q(
  `SELECT count(*)::int AS n FROM ingredient_evidence WHERE review_status='approved';`
);
const newPmids = q(
  `SELECT pmid FROM ingredient_evidence WHERE pmid IN ('37941097','12100180','10417566','17456614','18489289','19845667','14756523','21665869','15729863') ORDER BY pmid;`
);
console.log("CONCERNS", JSON.stringify(concerns.rows));
console.log("EV_COUNT", JSON.stringify(count.rows));
console.log("NEW_PMIDS", JSON.stringify(newPmids.rows));
console.log(JSON.stringify({ phase: "evidence_phase3_seed_ok" }));
