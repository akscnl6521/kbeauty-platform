...
현재 실제 진행 상태는 이 문서를 우선한다. 최상위 방향은 Master Plan **v4.3**을 따른다.

## 트랙 구조 (Master Plan v4.3 §44)

```text
[얼굴 MVP] 단계 1 → 2 → 3 → 4     ← 단계 4 완료 = MVP 런칭 시점
단계 5   리텐션
단계 5.5 두피·모발 트랙 (확장 A)        ← 신설, 미착수
단계 6   증상 기반 피부과 (얼굴 + 두피·모발 전문의)
단계 6.5 카테고리 확장 트랙 B           ← 신설, 미착수
단계 7 → 8 → 9
```

**얼굴 MVP 트랙(단계 1~4)은 완료**되어 2026-07-26 Production 출시했다(`www.kbeautymatch.com`).
두피·모발과 §12의 나머지 카테고리는 MVP 범위 밖이며, 단계 5.5·6.5에서 연다(§29).

### 신설 트랙 착수 조건

| 트랙 | 선행 조건 | 상태 |
|---|---|---|
| 단계 5.5 두피·모발 | 단계 5 리텐션이 실사용자 기준으로 안정화된 뒤 | 미착수 |
| 단계 6.5 카테고리 확장 B | 단계 6 피부과 트랙이 실데이터로 작동한 뒤. 바디케어부터 순차 | 미착수 |

여성 청결·구강 케어는 규제·의료 경계 검토 전까지 **보류**이며 추천 대상에 넣지 않는다.

### 카테고리 스코프 확장 (2026-07-28 지시)

스킨케어 외 아래 3개 카테고리를 **같은 원칙(§1~§24) 그대로** 확장한다. 추천 엔진·
안전 필터·루틴 로직은 기존 스켈레톤을 **재사용**하고 새로 만들지 않는다. 구분은
`products.category` 하나로 한다.

| 카테고리 | `products.category` 값 | 진행 순서 |
|---|---|---|
| 스킨케어 (기존) | `serum` `cream` `toner` `essence` `ampoule` `sunscreen` `cleanser` `foam_cleanser` `cleansing_balm` `moisturizer` `mask` `eye_cream` `eye_patch` | 1 |
| 뷰티 용품·도구 | `tool_puff` `tool_brush` `tool_device` `tool_container` | 2 |
| 메이크업 | `makeup/base` `makeup/color` `makeup/lip` `makeup/eye` | 3 |
| 두피·헤어케어 | `shampoo` `conditioner` `hair_treatment` `hair_oil` `scalp_ampoule` `hair_styling` | 4 |

**카테고리별로 달라지는 것과 그대로인 것**

| 영역 | 처리 |
|---|---|
| 추천 랭킹·안전 필터·루틴 | **그대로 재사용.** 점수 가중치·게이트 무변경 |
| 얼굴 트랙 풀 분리 | `isOutsideFaceTrack()` 로 얼굴 시나리오에서 제외 (§29). 카테고리별 트랙은 각자의 풀을 쓴다 |
| 브랜드 레지스트리 | 카테고리마다 브랜드가 다르므로 `brandRegistry.ts` 에 **추가**한다(§6). 기존 항목은 건드리지 않는다 |
| 전성분 정규화 | 스킨케어 규칙(§35.7)을 기본으로 쓰되, 카테고리 특유 표기가 나오면 규칙을 **추가**한다(§8). 기존 규칙 변경 금지 |
| 도구·기기 | **전성분이 없는 제품군이다.** 성분 게이트를 그대로 적용하면 전부 막힌다 — 카테고리별 게이트 분기가 필요하다(아래 «발견된 문제» 참고) |

**주의 — 도구·기기는 성분 게이트와 충돌한다.** 현재 활성화 게이트는
`hasOfficialIngredientsText` 를 필수로 요구한다(`product-activate.ts`). 퍼프·브러시·
클렌징 기기는 전성분이 없으므로 이 게이트를 통과할 수 없다. 게이트를 낮추는 것이
아니라 **카테고리별로 요구 증거를 달리하는** 설계가 필요하다. 미해결.
