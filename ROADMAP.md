...
- [~] 실공식 출처 live verify · verified 구매 SKU 풀 — **2026-07-25 실 라이브 크롤 1회 실행** (COSRX·Beauty of Joseon·ROUND LAB·Isntree·Anua 5개 브랜드 · robots-aware · 실제 38건 시도·30건 추출 성공·8건 staging_ready · Staging `product_discovery_candidates`에 22건 신규 upsert · Production 미접촉 · 사람 검수 전이라 공개/추천 미반영). 현재 Staging 실 공개 `products` 활성 **20개** (수천 개와는 거리가 큼) · discovery 후보 누적 1,197건 중 1,085건은 과거 placeholder 시드가 증거 미비로 정당하게 rejected · 실사람 검수→verified→published 전환은 여전히 `external_only`
