-- Complete Search-to-Verified-Product Pipeline after partial apply.
-- Context: remote migration create_search_to_verified_pipeline (20260713032925)
-- created public.brands only (structure OK, RLS OFF, default ACL open).
-- This migration does NOT recreate brands. It:
--   A) locks brands (RLS + REVOKE ALL + SELECT policy + GRANT SELECT)
--   B) creates the remaining 10 tables from the original design
--   C) enables RLS/policies/privileges for all 11 tables
-- Does NOT: DROP brands, alter products/ingredients/product_offers,
--           modify schema_migrations, or change existing brands columns.
-- Apply exactly once via apply_migration name: complete_search_to_verified_pipeline
-- Do NOT re-run if success is unclear.

-- =============================================================================
-- A. brands security lock (table already exists; structure matches original)
-- =============================================================================
...
-- B. No brands structural ALTER: remote columns/constraints/indexes already match original.

-- =============================================================================
-- C. Remaining 10 tables + D. full RLS/privileges (from original migration tail)
-- =============================================================================
-- =============================================================================
-- 2. product_variants
-- =============================================================================
