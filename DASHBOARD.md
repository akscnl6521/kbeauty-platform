...
## 55. 국내몰이 EUC-KR 이었다 — 한글이 통째로 깨져 있었다 (2026-08-05)

국내 오퍼가 «1건뿐» 이던 이유를 파고들어 **원인을 찾았다.** 대조 알고리즘 문제가
아니라 **읽는 단계에서 한글이 깨지고 있었다.**

### 증상

COSRX 몰 제품명이 이렇게 들어왔다:

```
AC 컬렉션 카밍 폼 클렌저 150ml   →   AC �÷��� ī�� �� Ŭ���� 150ml
```

깨진 이름은 대조 점수가 **전부 0.00** 이 된다. COSRX 는 우리 카탈로그의 절반
(26건 중 14건)이라 이게 국내 오퍼 확보를 통째로 막고 있었다.

### 원인

국내 쇼핑몰 상당수가 아직 **EUC-KR(CP949)** 로 서비스한다. `await response.text()` 는
무조건 UTF-8 로 읽는다. 헤더도 `text/html` 만 주고 charset 을 안 붙여서, `<meta charset>`
을 봐야 한다.

`decodeHtmlBody()` 를 만들어 헤더 → meta 태그 → UTF-8 순으로 인코딩을 정한다.
`ks_c_5601-1987` · `cp949` 같은 표기도 euc-kr 로 모은다.
회귀 테스트(`test:decode-html-body`)로 고정했다 — **조용히 틀리는 종류**라 테스트가 없으면
다음에 또 «국내몰에 그 제품이 없다» 는 잘못된 결론에 도달한다.

### 음역 사전 보강 — 실제 표기 쌍에서만

한글이 읽히자 어떤 낱말이 없는지 보였다. **몰의 실제 이름과 DB 이름 쌍에서 확인된
것만** 넣었다(각 항목 옆에 근거 제품을 적었다):

```
올인원=all in one · 미드나잇=midnight · 블루=blue · 굿모닝=good morning
시트=sheet · 핏=fit · 워터=water · 알콜=alcohol · 프리=free
서플=supple · 프레퍼레이션=preparation · 언센티드=unscented · 페이셜=facial
어드벤스드=advanced · 히아루론산=hyaluronic acid …
```

`페이셜` 은 **구분하려고** 넣었다 — 클레어스는 «페이셜» 판과 «언센티드» 판이 따로 있고,
둘을 섞으면 다른 제품이 붙는다.

기존 표기와 철자가 다른 것도 따로 넣었다 — `어드밴스드`/`어드벤스드`,
`히알루론산`/`히아루론산` 은 몰마다 다르게 쓴다.

### 한 몰 상품은 한 제품만 뒷받침한다

보강 뒤 **잘못된 짝이 하나 생겼다.** `Black Snail All In One Cream`(77)이
`스네일 92 올인원 크림` 에 0.83 으로 붙었는데, 그건 `Advanced Snail 92`(28)의 상품이다
(28 은 0.86). 제품마다 따로 최고점을 고르면 이런 충돌을 못 본다.

(제품 × 상품) 짝을 **점수 높은 순으로 훑으며 하나씩 확정**하도록 바꿨다. 이미 쓰인
상품은 다음 제품이 가져갈 수 없다.

처음엔 URL 로 중복을 막았는데 **뚫렸다** — 같은 상품이 카테고리마다 다른 URL 로
노출된다(`branduid=202` 가 `scode=001`·`scode=005` 로 두 번). **상품명**으로 바꿔 막았다.
...
늘어난 폭이 크진 않다. 몰에 있어도 **품절**인 것이 많고(클레어스 미드나잇 블루·서플
프레퍼레이션 둘 다 품절), Round Lab 은 86개 전부 `availability` 를 안 준다.
**품절을 재고 있음으로 바꿔 넣지 않았다.**

### 검증

CI 회귀 40개 통과 · `tsc`·`lint` 무경고 · `build` 통과.
추천 풀 26건 · 국내 구매 가능 14건 · 이미지 26/26.

---

## 56. 사이트맵에서 목록 페이지를 제품으로 세고 있었다 (2026-08-05)

아누아 몰이 «제품 34개» 로 보이는데 가격 정보가 0건이었다. 열어 보니 **34개가 전부
목록 페이지**였다.

```
http://anua.kr/product/list.html?cate_no=24        ← 카테고리 목록
http://anua.kr/board/product/list.html?board_no=4  ← 후기 게시판
```

URL 필터가 `/product/` 만 보고 걸러서 이런 것까지 제품으로 잡았다. 목록을 제품으로 세면
두 가지가 나빠진다 — **쓸데없는 요청으로 상대 서버에 부담**을 주고(차단당하면 다음
수집이 아예 안 된다), **«이 몰은 가격을 안 준다» 는 잘못된 결론**에 도달한다.

`extractProductUrlsFromSitemap()` 으로 옮기고 목록·게시판·검색·이벤트를 제외했다.
`&amp;` 같은 XML 엔티티도 여기서 푼다 — 안 풀면 URL 이 깨져 요청이 실패한다.
회귀 테스트(`test:mall-sitemap`)는 실측 URL 로만 만들었다.

### Round Lab 은 재고를 알 수 없다 — 추정하지 않는다

86개 전부 `availability` 가 없는 이유를 확인했다. JSON-LD `offers` 에 그 필드가
**아예 없다**:

```json
"offers":{"@type":"Offer","url":"…","priceCurrency":"KRW","price":23000}
```

본문에 `구매하기`·`장바구니` 는 있고 `품절` 은 없지만, **품절 페이지를 본 적이 없어
그 신호가 재고를 뜻하는지 검증할 수 없다.** 검증 못 한 신호로 `in_stock` 을 넣으면
구매 버튼을 눌러도 못 사는 제품이 생긴다. **그대로 둔다.**

### 현재 국내 오퍼 상황

```
COSRX      가격 57 · 재고 53   ← 실질적으로 여기서만 확보된다
Abib       가격 96 · 재고 23
Klairs     가격 71 · 재고 17   (우리 제품 2건은 둘 다 품절)
Round Lab  가격 86 · 재고  0   (재고 정보 없음)
```

국내 구매 가능 **14건**. 늘리려면 몰이 더 필요한데, 사이트맵으로 제품을 주는 국내
공식몰이 이 4곳뿐이다. 아누아·토리든·SKIN1004 는 사이트맵에 목록 페이지만 있어
제품 링크를 목록에서 따로 긁어야 한다 — 그건 몰마다 다른 파싱이 필요하다.
...
## 57. 방향 전환 — **국내에 파는 것부터 등록한다** (2026-08-05)

사용자 지적: «국내에 파는 것만 우선 만들고 싶다». 그동안 방향이 반대였다.

지금까지는 «우리 DB 제품 → 국내 오퍼 찾기» 였다. 그러면 DB 제품이 국내몰에 없으면
아무것도 못 한다. 실제로 추천 풀을 17 → 26건으로 늘렸는데 **국내 구매 가능은 13 → 14건**
에 머물렀다. 늘어난 12건이 전부 미국 오퍼뿐이라 **국내 사용자에게는 안 보였다.**

**반대로 간다 — 국내몰에 실제로 파는 것부터 등록한다.**

### 국내몰 하나에서 필요한 게 전부 나온다

```
제품명(한글) · 전성분 · 가격(원) · 재고 · 공식 이미지 · 구매 링크
```

COSRX 국내몰이 전성분을 한글로 싣는다는 것을 확인한 것이 출발점이었다:

```
전성분  정제수, 글리세린, 스테아릭애씨드, 미리스틱애씨드, 피이지-32, …
```

### 막고 있던 것 — 사전에 한글명이 없었다

등록해 보니 미매칭이 제품당 8~24개였고, 빠지는 것이 `부틸렌글라이콜`(22회)·
`글리세린`(21회)·`잔탄검`(14회) 같은 **가장 흔한 성분**이었다.

원인은 **성분이 없는 게 아니라 한글 이름이 없는 것**이었다. 사전이 영문 제품 위주로
만들어져 `name_ko` 가 빈 행이 290개였다. 새 행을 넣으면 같은 성분이 둘이 되므로
**기존 행의 한글명을 채웠다**(식약처 원료성분정보, 184행).

### 남은 미매칭은 파싱 잔여물이었다

```
Disodium EDTA 정제수            ← 영문 목록 끝 + 한글 목록 시작이 붙음
다이소듐이디티에이 사용할 때의      ← 안내 문구가 붙음
```

국내몰은 전성분을 **영문·한글 두 벌**로 싣는다. 라벨 뒤 구간을 잘라 오면 두 목록이
한 토큰에 붙는다. 성분명 하나에 라틴과 한글이 섞이는 경우는 없으므로 **문자 종류가
바뀌는 지점을 목록 경계**로 보고 가른다. 안내 문구는 §40 의 `stripIngredientNoticeTail`
을 카탈로그 쪽에도 적용했다.

이걸 고치자 미매칭 8~24 → **1~5**, 활성화 가능 0 → **11건**.

### 성분 링크가 없어 게이트에서 막혔다

`structured_ingredients_missing` — 게이트는 `product_ingredients` 링크를 요구하는데
새 제품엔 없었다. 기존 링크 생성기는 대상이 옛 수집 artifact 에 묶여 새 제품을 못 본다.
**DB 를 보고 «전성분은 있는데 링크가 없는» 제품**을 고르는 스크립트를 만들었다(918행 생성).
...
| 이미지 | 26/26 | **36/36** |
| 전성분 검증 | 26/26 | **36/36** |
| 브랜드 | 10개 | **11개** (Abib 9건 신규) |

**국내 구매 가능이 14 → 24건.** 이번이 국내 사용자에게 실제로 제품이 늘어난 첫 작업이다.

### 등록 원칙

  · 전성분이 없거나 검증을 통과 못 하면 **등록하지 않는다**
  · 재고가 `InStock` 이 아니면 등록하지 않는다 — 살 수 없는 것을 권하지 않는다
  · 이미 있는 제품과 이름이 겹치면 새로 만들지 않는다
  · 브랜드명은 몰의 한글 표기 대신 **우리 카탈로그의 영문 표기**를 쓴다(§35.3)
  · `active`/`verified_at` 은 **게이트가 정한다** — 우회하지 않는다
