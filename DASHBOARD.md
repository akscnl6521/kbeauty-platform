...
최종 갱신: 2026-07-27 (자동 갱신 — 채팅 보고 대신 이 파일을 최신 상태로 유지)

> 영상·사용가이드 트랙(§26)의 현재 상태는 **§26-21**에 요약돼 있다.
...
### 26-14. 사용 가이드 18건 적재 완료 (→ §26-19에서 22건으로 늘어남)
...
### 26-21. 영상·사용가이드 트랙 현재 상태 (2026-07-27 · 대기 중)

브랜치 `feature/video-infrastructure-20260727`, main 대비 24 커밋. **대기 상태** —
사장님 검수 결과와 다른 두 트랙 진행 상황을 보고 다음이 정해진다.

| 항목 | 상태 | 남은 것 |
|---|---|---|
| §36.4 영상 스키마 (9테이블 + 뷰) | Staging 적용·검증 완료 | 자산 0건 |
| §36.5 사용 가이드 | **Staging 22건, 전부 `needs_review`** | **사람 검수 대기** |
| 카테고리 공통 영상 | 0건 확보 | 자체 제작 필요 — 보류 |
| `/admin/media-review`·`/admin/usage-guides` | 완료, 접근성 감사 통과 | — |
| §41 재확인 워커 | 완료 (수동 실행) | 자동 스케줄 등록은 운영자 몫 |
| 사용법 미확보 49건 | OCR 필요로 확정 | 정확도 기준 합의 후 판단 |
| 병합 준비 자료 (C) | 미착수 | 병합 논의 시점에 지시 예정 |

**막고 있는 것**: 사용 가이드 22건 검수(`/admin/usage-guides`). 승인해도 사용자
화면에는 나타나지 않으므로 부담 없이 진행 가능하다.

**다음 트랙(카드에 영상·가이드 삽입) 전제 조건**: 디자인 트랙이
`/routine`·`/results`에 도달해야 하고, 삽입할 콘텐츠가 있어야 한다. 현재 영상은
0건이므로 텍스트 가이드가 먼저다.

**검증 방법**: `npm run test:video-track` (오프라인 10종). Staging이 필요한 것은
`verify:media-library-staging` · `verify:usage-guides-staging` ·
`e2e:usage-guide-review-staging` · `media:recheck`.
...
### 26-23. 얼굴 일치 검증 + 공통 안내 문구 (2026-07-27)

튜토리얼 시리즈(세안 → 스킨케어 → 메이크업) 착수 조건 2가지를 처리했다.

#### 1. 얼굴 일치 검증 — 규칙·기록은 완성, 판정은 사람이 한다

**자동 얼굴 인식은 이 환경에서 만들 수 없다.** 얼굴 임베딩 모델도, 비전 API도
없다. 그래서 만든 것은 «자동 판정기»가 아니라 **판정을 반복 가능하고 기록되게
만드는 장치**다. 픽셀 비교는 사람이 후보 프레임과 레퍼런스를 나란히 놓고 한다.

`src/lib/media/modelIdentityCheck.ts` — 판정 규칙(순수):

| 발견 | 판정 |
|---|---|
| 금지된 변형 1개라도 (턱 뾰족·눈 확대·코 변형·연령 변경·타인화) | **fail** — 9개 항목이 전부 match여도 |
| 고정 항목 1개라도 불일치 | **fail** |
| 판독 불가 또는 미평가 항목 존재 | **inconclusive** — 통과 아님 |
| 레퍼런스 0장 대조 | **inconclusive** |
| 9개 전부 match + 변형 없음 + 레퍼런스 대조함 | **pass** |

고정 항목 9가지는 임의로 정하지 않았다 — 레퍼런스를 생성할 때 쓴 프롬프트의
정체성 고정 조항 그대로다(눈매·눈동자 색·눈썹 형태·코의 길이와 폭·입술 모양·
턱선·얼굴형·피부톤·헤어라인).

**«판독 불가»를 «통과»와 분리한 것이 핵심이다.** 흐릿하거나 각도가 안 맞아
판단할 수 없는 프레임은 통과한 프레임이 아니다. 이걸 통과로 처리하는 것이
얼굴이 서서히 바뀌는 걸 놓치는 경로다.

**프레임당 최소 3장과 대조**한다. 레퍼런스가 완전 측면부터 정면까지 걸쳐
있어서, 1장만 보면 각도 차이인지 인물 차이인지 갈리지 않는다.

```
npm run media:identity-refs                        # 레퍼런스 목록 + 확인할 항목
npm run media:check-identity -- --calls calls.json # 판정 채점 + 기록
npm run test:model-identity-check                  # 규칙 회귀 테스트
```

판정은 `media_review_events`에 남는다. **통과해도 자동 승인은 아니다** —
자산은 `needs_review`로 유지된다.
...
`CLOSING_NOTICE_KO` 상수로 고정했다. 클립마다 표현이 조금씩 달라지지 않도록
한 곳에서만 정의한다. §36.3 «AI 생성 콘텐츠» 표기는 별개이며 **재생 내내**
노출돼야 한다(`videoOverlayRequirements()`).

> §14 검토에서는 «피부과 확인 우선» 문구도 함께 제안했으나, 사용자가 짧은 한
> 줄로 범위를 확정했다. 이번 시리즈는 치료가 아니라 기법 튜토리얼이므로
> 그 판단으로 진행한다.

#### 3. 남은 차단 요인 — 영상 생성 수단

위 2가지가 끝나도 **영상 자체를 만들 수 없다.** 생성 API 미연동, ffmpeg 미설치,
프레임 디코딩 불가. 이건 이 트랙에서 해소할 수 없고 별도 결정이 필요하다.
