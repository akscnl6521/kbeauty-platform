...
식약처 «화장품 원료성분정보» 는 한글명만 있고 영문명이 빈 행이 있다.  은 NOT NULL 이라 그대로는 넣을 수 없고, 영문명을 지어내는 것은 §17 위반이며  에 한글을 넣는 것은 이번 세션에 정리한  오염의 반복이다.

**대한화장품협회 성분사전(kcia.or.kr/cid)에서 표준 영문명을 조회했다.** 조회 경로는  이다( 는 숫자가 아니라  같은 코드다).
...
| 6463 | 광엽발계뿌리추출물 | **Smilax Glabra Root Extract** (구명칭 중국토복령추출물) |
 ...
 의 표준 영문명은 **** 다. 빙하에서 왔다는 사실은 표준 명칭에 반영되지 않는다 — 우리가 «Glacier Water» 로 바꾸면 표준을 벗어나므로, 기존  행에 **별칭으로** 붙였다.

 에 성분코드와 함께 적어 나중에 대조할 수 있게 했다. 협회 사전에도 없으면  로 남기고 넘어간다 — 지어내지 않는다.
