...
| 제품 | 막는 성분 | 상태 |
|---|---|---|
| 84 · 85 아도르 원더 수분 샴푸 | `빙하수` | 식약처에 **있으나 영문명이 비어 있음** |
| 83 아도르 더마티컬 헤어로스 샴푸 | `광엽발계뿌리추출물` | 〃 (`오미자/광엽발계뿌리추출물`) |
| 53 Laneige 틴티드 립 세럼 | `비스-베헤닐/아이소스테아릴/피토스테릴다이머다이리놀레일다이머다이리놀리에이트` | 식약처에 영문명까지 있는데 적재 대상에 안 잡혔다 — 재확인 필요 |

**`ingredients.name_en` 은 NOT NULL 이다.** 영문명 없는 성분을 넣으려면 name_en 을 채워야 하는데, 식약처가 주지 않는 값을 지어낼 수 없다. `name_en` 에 한글을 넣는 것은 `ko-batch` 행들이 저지른 오염이고 이번 세션에 정리한 대상이다 — 반복하지 않는다.

선택지는 두 가지이며 **둘 다 사람 판단이 필요하다**: `name_en` 을 nullable 로 바꾸는 migration, 또는 영문명을 다른 공식 출처(INCI 사전)에서 확인해 채우기.
