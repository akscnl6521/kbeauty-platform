...
최종 갱신: 2026-08-09 (추천 풀 128건 · 브랜드 18개 · 반려 31 → 17건)
...
## 2026-08-09 (8차) 「사람이 봐야 한다」 로 미뤄둔 31건 — 절반이 우리 결함이었다

next_task: 남은 5개 시나리오. `kr-acne-spot-treatment` 와
`kr-acne-mattifying-emulsion` 은 매칭 0 — 여드름 스팟·매트 에멀전을 파는 몰이 필요하다.

### 검수로 넘기기 전에 이유부터 봤다

안전 검증기가 «목록 끝 경계가 애매하다» 로 반려해 둔 것을 사람에게 넘기려다,
**어느 토큰에서 잘렸는지**를 먼저 찍게 했다. 그랬더니:

| 잘린 자리의 토큰 | 정체 |
|---|---|
| `꿀` (6건) | **벌꿀** — 한 글자 한글 성분명 |
| `토마토열매/잎/줄기추출물` 등 (5건) | 표준 INCI 표기 — `잎` 이 한 글자라 떨어졌다 |
| `나일론6/12` | 슬래시 뒤가 숫자만이라 떨어졌다 |
| `제품명: …` · `유기농 재배 원료 …` | 진짜 쓰레기 (정상 반려) |

**검수가 아니라 형태 판정기의 결함이었다.** 판정기를 «더 느슨하게» 한 게 아니라
**진짜 성분 형태를 알아보게** 고쳤다 — 쓰레기는 여전히 반려된다(테스트로 고정).

  전성분 반려 31 → 17건 · 되찾은 제품 14건

### 사전 적재를 막던 가짜 충돌 둘

`꿀` 은 식약처에 `Honey` 로 있는데도 안 들어갔다. 두 겹으로 막혀 있었다:

1. **slug 를 «점유된 키» 로 셌다.** 31번 «Honey Extract» 의 slug 가 `honey` 라서
   전혀 다른 성분인 «Honey»(꿀)가 충돌로 판정됐다. slug 는 URL 용이고
   성분 대조에 쓰이지 않는다 — 매칭 키에서 뺐다.
2. **slug 가 겹치면 성분을 버렸다.** 이제 `-2` 를 붙여 다른 이름을 준다.
   URL 키 하나 때문에 성분을 못 넣을 이유가 없다.

설화수 5개 제품(진설크림·윤조에센스)이 `꿀` 하나 때문에 막혀 있었다.
...
## 2026-08-09 (7차) 오퍼가 낡아도 아무도 모르는 상태였다 (단계 8)

next_task: 이 재확인을 **주기적으로** 돌릴지 운영자가 정한다. 지금은 사람이
`npm run check:kr-offer-freshness -- --apply` 를 돌려야 갱신된다. 자동 스케줄
등록은 이 저장소 규칙상 에이전트가 하지 않는다.

### 무엇이 문제였나

구매 CTA 관문(`isOfferPurchasableForCta`)은 `verifiedAt` 이 **있는지**만 보고
**얼마나 오래됐는지는 보지 않는다.** 2년 전 값도 그대로 «구매하기» 로 나간다.

기존 `refresh:product-daily` 는 `mode: "fixture"` 로 도는 **뼈대**다 — 산출물
형식만 만들고 실제 데이터를 다시 확인하지 않는다. 즉 오늘 넣은 가격·재고가
낡아도 알아차릴 장치가 없었다. 가격이 바뀌면 화면이 **조용히** 틀려진다.

### `check:kr-offer-freshness`

국내몰 페이지에서 가격·재고를 다시 받아 저장된 값과 견준다.

  · **재고를 명시하지 않으면 재고를 바꾸지 않는다** — 표기 없는 것을 «판매중»
    으로도 «품절» 로도 읽지 않는다(이 저장소의 기존 결정과 같다)
  · **페이지를 못 받으면 아무것도 바꾸지 않는다** — 일시 장애로 멀쩡한 판매처를
    지우면 안 된다
  · 가격이 자리표시(1,000원 미만)면 반영하지 않는다

첫 실행: 국내몰 오퍼 **97행 전부 «그대로»**. 오늘 넣은 값이 정확하다는 뜻이다.

**사진 주소도 같이 본다.** 몰이 상품 페이지를 갈아엎으면 이미지가 404 가 되고
화면에는 깨진 자리만 남는데, 아무 오류도 안 나서 알 수가 없다. 전수 확인 결과
화면에 나가는 사진 **166건 전부 열린다**. 안 열리는 것이 생기면 지우지 않고
`validation_status` 만 내려 화면에서 빼고, 다시 열리면 되살릴 수 있게 둔다.

`gate:prelaunch` 에 «오퍼를 마지막으로 확인한 시점» 을 **보고 항목**으로 넣었다.
신선도를 자격 조건으로 바꾸는 건 국내 오퍼 규칙을 바꾸는 일이라 사람이 정한다 —
여기서는 숫자만 보인다.

## 2026-08-09 (6차) 출시 관문(WQ-G)이 파일 3개만 보고 있었다

next_task: 운영자 판단이 필요한 것 두 가지 — (1) `gate:prelaunch` 보고 항목 중
«국내 구매 가능 105/116» 을 출시 기준으로 삼을지, (2) 안전 검증기가 반려해 둔
제품 36건(«목록 끝 경계가 애매하다»)을 사람이 볼지.

### 무엇을 찾았나

WQ-G 의 관문은 `check:production` 과 `check:release-security` 두 개다.

  `check:release-security` (84줄) — **내용이 충실하다.** 서비스 키가 클라이언트로
      새는지 · 하드코딩된 JWT·비밀번호 · `.env.local` 추적 여부 ·
      파이프라인 파괴 플래그 · mock 공급자 가드.

  `check:production` (25줄) — **파일 세 개가 있는지와 npm 스크립트가 정의됐는지**를
      본다. 「출시해도 되는가」 에는 답하지 못하는데, 이름 때문에 답하는 것처럼 보인다.

### `gate:prelaunch` 를 만들었다

**사용자가 보는 것**을 잰다 — DB 행 수가 아니라 라이브 API 응답. 오늘 「이미지
84/84」 라고 보고했지만 화면에는 한 장도 안 떴던 일이 있었고, DB 를 세면 그런 걸
못 잡는다.

**차단**과 **보고**를 나눈다. 차단은 «이대로 나가면 사용자가 잘못된 것을 본다»
뿐이고, 나머지는 숫자만 준다 — 출시 기준을 스크립트가 정하지 않는다.

첫 실행에서 바로 두 개를 잡았다:

  · 이미지 116/118 — 새로 등록한 COSRX 2건이 `http://` 로 저장돼 화면에서 차단됨.
    **등록 경로가 여전히 http 로 담고 있었다**(고친 것은 이미 담긴 행뿐이었다).
  · `오일 컨트롤 에센스 30ml` 와 `수분 크림 80ml` 의 전성분 39개가 **순서까지 동일**.
    에센스와 크림이 같은 처방일 수 없다 — 둘 다 내렸다(77번과 같은 사례).
...
|---|---|
| 주요 화면이 열린다 | 6종 200 |
| 추천 풀에 제품이 있다 | 118건 · 브랜드 17개 (클라뷰 추가) |
| 화면에 사진이 뜬다 | **118/118** · http 0 · 공유 사진 0 |

| 보고 항목 | 값 |
|---|---|
| 국내 «구매하기» | 107 / 118건 |
| 출처 다른데 전성분 같은 묶음 | 2개 (달바 150/160ml · 셀리맥스 토너 150/320ml — 용량만 다른 같은 제형으로 보인다) |

## 2026-08-09 (5차) 176건이 «이미 있는 제품» 으로 조용히 버려지고 있었다

next_task: 남은 5개 시나리오. 그중 `kr-acne-spot-treatment` 와
`kr-acne-mattifying-emulsion` 은 **매칭 0** 이라 그 유형을 파는 몰을 찾아야 한다.
나머지 3개는 제품은 있으나 오퍼·전성분이 없다(해당 브랜드 몰이 사이트맵을 안 낸다).

### 무엇을 찾았나

등록 스크립트가 «이미 있는 제품» 이라며 **176건을 버리고 있었는데, 그 판정을
아무 데도 기록하지 않았다.** 왜 후보에 안 올라오는지 알 방법이 없었다.

기록을 남기게 하고 보니 유사도 기준이 **0.7** 이었다. 한 브랜드 라인은 이름
대부분을 공유해서(달바는 거의 전부 «화이트 트러플 …») 서로 다른 제품이 붙는다:

    딥 클린 폼 클렌저            ↔ 리턴 오일 크림 클렌저 160ml        0.75
    갈락토미세스 95 톤 밸런싱 에센스 ↔ 갈락토미세스 95 화이트닝 파워 에센스  0.75
    너리싱 트리트먼트 마스크        ↔ 슬리핑 마스크                    0.75

유사도 분포를 보니 **1.00 이 132건**(진짜 중복)이고 나머지 44건이 0.71~0.86 이었다.
기준을 0.95 로 올리고, 포장·수량만 다른 경우는 `packagingNeutralKey` 로 따로 잡았다.

### 교훈

**버리는 판정도 기록해야 한다.** 통과한 것만 찍으면, 잘못 버려도 드러나지 않는다.
이번 세션에서 같은 성격의 결함을 세 번 만났다 — 이미지 연결 누락, 진행도 계기판,
그리고 이 중복 판정. 셋 다 «조용히 아무 일도 안 일어나는» 형태였다.

## 2026-08-09 (4차) 활성 과제의 진행도 계기판이 고장나 있었다

next_task: 남은 5개 시나리오 — `kr-acne-spot-treatment`(매칭 0) ·
`kr-acne-mattifying-emulsion`(매칭 0) · `kr-acne-sebum-cleanser`(2, 이미지·오퍼) ·
`kr-redness-gentle-cleanser`(1, 오퍼) · `kr-redness-calming-mist`(1).
스팟 케어·에멀전은 지금 몰에 거의 없다 — 그 유형을 파는 몰을 겨냥해 찾아야 한다.


### 8개 중 3개를 채웠다 — 대부분 «없어서» 가 아니라 «못 알아봐서» 였다

| 무엇을 고쳤나 | 효과 |
|---|---|
| 시나리오 어휘와 카탈로그 표준 분류가 **다른 말을 쓰고 있었다** | +1 |
| 자외선 고민을 유형이 `sunscreen` 일 때만 붙였다 | +1 |
| **사전 한 칸에 이름이 여럿** 들어 있었다 | +1 (제품 8건 활성화) |

**시나리오는 `mist`·`moisturizer`·`spot_treatment` 라는 굵은 이름을 쓰고, 카탈로그
표준은 `facial_mist`·`cream`·`spot_care` 로 잘게 나눈다.** 어제 유형을 표준으로
맞추면서 이 어긋남이 드러났다 — 데이터를 한쪽으로 옮기는 대신 **매칭에서 잇는다**.
두 어휘 다 이유가 있다(시나리오는 사용자 언어, 카탈로그는 유통 표준).

**선 미스트가 자외선 시나리오에서 통째로 빠져 있었다** — 유형이 `sunscreen` 일
때만 `uv` 를 붙였기 때문이다. 선 미스트·선스틱도 자외선 제품이다.

**Abib 토너패드 5건이 `Cynanchum Atratum Extract` 하나에 막혀 있었는데, 그 성분은
사전 895번에 이미 있었다.** 영문명이 `Vincetoxicum Atratum Extract,Cynanchum
Atratum Extract` 로 **쉼표로 이어져** 있어서 통째로 한 이름으로 취급됐다.
식약처가 동의어를 그렇게 준다. 쉼표로 나눠 각각을 이름으로 등록했다 —
`1,2-Hexanediol` 처럼 숫자 사이 쉼표는 이름의 일부라 쪼개지 않는다.
