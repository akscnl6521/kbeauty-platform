...
최종 갱신: 2026-08-15 (시나리오 29/30 · 추천 풀 136건 · 브랜드 20개)
...
## 2026-08-15 아크로패스로 여드름 스팟 시나리오를 채웠다 — 29/30

next_task: **몰 확대는 소진됐다.** 아래 «마지막 시나리오» 를 보고 운영자가
브랜드를 짚어 주는 편이 빠르다. 그 밖에는 운영자 결정 세 가지가 남아 있다.

### 마지막 시나리오 — 지금은 정직하게 못 채운다

`kr-acne-mattifying-emulsion` 은 **여드름 고민 + 에멀전 유형**을 요구한다.
확보한 20개 몰을 다시 훑은 결과:

| 찾은 것 | 왜 못 쓰나 |
|---|---|
| `디에이지 레드 에디션 에멀젼` (등록됨) | 안티에이징 라인이라 **여드름 고민이 아니다** |
| 달바 `펩타이드 노세범` 4종 (피지 조절) | 페이지에 **`전성분` 도 `정제수` 도 0회** — 진짜로 표기가 없다 |
| 셀리맥스 `오일 컨트롤` 2종 | 전성분 39개가 순서까지 같아 **둘 다 내려 둔 상태** |
| `[선착순 비밀특가] 오일 컨트롤 …` | 행사가라 단가가 아니다 |

억지로 채우지 않는다 — 안티에이징 에멀전을 여드름 칸에 넣으면 숫자는 30/30 이
되지만 사용자가 받는 추천은 틀린 것이 된다. **29/30 이 정직한 값이다.**

### 겨냥해서 찾았다

비어 있는 시나리오가 «여드름 스팟» 이라는 걸 알고 있었으므로, 더마·여드름 브랜드
20곳을 겨냥해 확인했다. **아크로패스**(트러블 패치 전문)가 통과했다.

  후보 202곳 누적 · 이번 20곳 중 2곳 통과 (아크로패스 · 라비오뜨)

유세린·아벤느·라로슈포제·세타필·피지오겔은 **전부 사이트맵이 없다** — 국내 법인
사이트가 카탈로그를 안 낸다.

### 등록 필터에 또 걸린 것

아크로패스 후보 10건 중 5건이 **같은 제품의 개수 변형**이었다 —
`(6패치)` · `(12패치)` · `(24패치)` · `(36패치)`. 포장 중복 규칙이 `개`·`매` 는
지우는데 `패치` 는 몰랐다. 개수 낱말은 상품마다 다르므로(개·매·정·포·팩·패치)
보이는 대로 넓혀 간다.

`[40% 역대급 할인]` 도 막았다 — 상품명에 «할인» 이 붙으면 그 값은 행사가다.

### 트러블 패치는 스팟 케어다

패치 3건이 **유형이 비어** 두 시나리오에 다 걸려 있었다(가짜 매칭).
`트러블큐어 마이크로콘 패치` 처럼 **무엇을 위한 패치인지**가 이름에 있을 때만
`spot_care` 로 본다 — 토너 패드와 헷갈리지 않게.
...
## 2026-08-11 운영 안내서 — 도구가 여섯 개인데 언제 뭘 돌리는지가 흩어져 있었다

next_task: 남은 2개 시나리오(여드름 스팟·매트 에멀전). 그리고 운영자 결정 두 가지
— 재고 미표기 몰 6곳(270건+)을 쓸지, 국내 구매 비율을 출시 기준으로 삼을지.

### `docs/159-kr-mall-catalog-runbook.md`

08-08~10 에 만든 도구를 **언제 무엇을 위해 돌리는지** 한자리에 모았다.

  주 1회      `check:kr-offer-freshness --apply` — 가격·재고·사진이 낡았는지
  배포 전     `gate:prelaunch` — 사용자가 보는 것이 멀쩡한지
  몰 추가 시   probe → snapshot → register → links → dictionary → activate → images
  이상할 때    duplicate-formulas · purchase-cta · scenario-catalog-gap

각 숫자가 무슨 뜻이고 이상하면 무엇을 하는지, **운영자만 정할 수 있는 것**은
무엇인지(재고 미표기 몰 · 출시 기준 · 반려 제품 검수) 적었다.

### 안내서도 조용히 썩는다

안내서는 사람이 그대로 따라 치는 문서다. 스크립트 이름을 바꾸면 **문서가 조용히
거짓말이 된다** — 문서는 아무 오류도 내지 않는다.

`test:runbook-commands` 를 만들어 CI 에 넣었다. 안내서에 적힌 npm 명령과 파일이
실제로 있는지 본다. **일부러 깨뜨려 실제로 실패하는 것까지 확인했다** — 통과만
하고 아무것도 안 잡는 검사는 없느니만 못하다.

## 2026-08-10 (3차) 같은 결함을 내 쓰기 스크립트 전부에서 찾아 고쳤다

next_task: 남은 2개 시나리오(여드름 스팟·매트 에멀전) — 그 유형을 파는 몰이 필요하다.

### 하나 고쳤다고 끝난 게 아니었다

유형 갱신이 «2건 바꿨다» 고 보고하고 실제로는 0행을 바꾼 걸 찾은 뒤, **내가 만든
쓰기 스크립트 전부**를 같은 눈으로 훑었다:

| 스크립트 | 갱신 | 행 수 확인 |
|---|---|---|
| `repair-media-validation-status` | 1 | **없었음** |
| `repair-media-http-urls` | 1 | **없었음** |
| `recheck-kr-mall-offers` | 2 | **없었음** |
| `deactivate-wrong-data-products` | 1 | 있음 |
| `backfill-ingredient-korean-names` | 1 | 있음 |

세 개가 «오류가 안 났다» 를 «바꿨다» 로 읽고 있었다. 전부 `.select("id")` 로
**바뀐 행을 돌려받아 세도록** 고쳤다. 0행이면 그렇게 말한다.

이 결함이 위험한 이유는 **되돌릴 수 없어서가 아니라, 안 됐다는 걸 모르기 때문**이다.
「이미지 84/84」 도, 「2건 바꿨다」 도 전부 그렇게 나온 보고였다.

## 2026-08-10 (2차) 고민 추론 힌트가 영어뿐이라 108건이 시나리오에 안 잡혔다

next_task: 남은 2개 시나리오 — `kr-acne-spot-treatment` · `kr-acne-mattifying-emulsion`.
가짜 매칭을 걷어내고 보니 **진짜로 제품이 없다.** 여드름 스팟·매트 에멀전을 파는
몰을 겨냥해야 한다.

### 무엇을 찾았나

추천 풀 131건 중 **108건이 `skin_concern` 이 비어 있었다** — 전부 국내몰에서
등록한 제품이다. 그런데 원인은 그 칼럼이 아니었다.

시나리오 대조는 성분 이름으로 고민을 미루어 보는데, **힌트가 영어뿐이었다**
(`niacinamide` · `panthenol` · `centella` …). 국내 제품은 전성분이 한글이라
(`나이아신아마이드` · `판테놀` · `병풀추출물`) **하나도 안 걸렸다.** 주요 성분은
다 갖고 있는데도 고민이 비어 어떤 시나리오에도 안 잡혔다.

한글 표기를 넣었다 — 표기가 여럿인 성분은 여럿 다 넣었다(`살리실릭애씨드`/`살리실산`).

  시나리오 준비 **25 → 28 / 30** · 시나리오-제품 연결 852개

### 유형이 비어 있으면 «아무 시나리오에나» 걸린다

유형이 없으면 대조에서 유형 조건이 통째로 건너뛰어진다. `지우개 폼 클렌징` 이
여드름 스팟·에멀전 시나리오에 잡혀 있었다 — 그래서 «매칭 3건» 처럼 보였지만
가짜였다. **이름이 유형을 분명히 말할 때만** 채우도록 했다(2건).

### 또 조용히 아무 일도 안 일어났다

유형을 채우는 갱신이 **0행을 바꾸고도 «2건 바꿨다» 고 보고**했다.
`.eq(칼럼, null)` 은 SQL 에서 NULL 을 못 잡는다(`= NULL` 은 참이 되지 않는다).
`.is()` 로 고치고, **바뀐 행 수를 세어서 보고**하게 했다.

오늘만 다섯 번째다 — 이미지 연결 · 진행도 계기판 · 중복 판정 · 내 편집 · 이 갱신.

## 2026-08-10 내 필터를 스스로 감사하고, 추출기 결함 둘을 찾았다

next_task: 남은 5개 시나리오(스팟·에멀전은 매칭 0). 그리고 «전성분 없음» 200건 중
표본 18건을 열어 보니 16건은 **정말 표기가 없다**(달바 128건은 이미지로만 싣는다).

### 내 필터가 오탐을 내고 있지 않은지 먼저 봤다

오늘 하루 «조용히 버려지는 것» 을 계속 찾았으니, **내가 만든 필터도 같은 의심을
받아야 한다.** 세 통을 열어 봤다:

| 필터 | 건수 | 결과 |
|---|---|---|
| 얼굴 스킨케어가 아님 | 127 | **오탐 없음** — «세럼 커버 쿠션» 처럼 이름에 세럼이 있어도 전부 색조다 |
| 이미 있는 제품으로 봄 | 139 | **오탐 없음** — 전부 유사도 1.00 |
| 세트 상품 | 64 | **오탐 없음** — `본품+리필` · `A+B` 전부 진짜 묶음 |

### 추출기 결함 둘

1. **«전성분은 물·글리세린으로 시작한다» 를 조건으로 걸어 두었다.**
   달팽이 크림은 `달팽이점액여과물` 로 시작한다. 그런 목록이 통째로 «전성분 없음»
   으로 버려졌다. **거의 모든 화장품에 들어가는 성분**으로 넓혔다 — 마케팅 문단은
   여전히 안 뽑힌다(테스트로 고정).

   이 동작은 자체 테스트가 «의도한 변경인지 확인할 것» 이라고 붙잡고 있었다.
   좋은 장치였고, 이유를 적어 함께 고쳤다.

2. **`&amp;` 를 안 풀고 있었다.** `Official &amp; Verified Product` 가 그대로 남는다.
   자체 테스트는 이미 잡고 있었는데 **CI 목록에 없어서 드러나지 않았다.**
   기본 다섯 엔티티를 맨 마지막에 푼다(`&amp;` 를 먼저 풀면 `&amp;lt;` 가 `<` 가 된다).
   두 선택자를 CI 에 넣었다.
