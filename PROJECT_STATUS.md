...
최종 갱신: 2026-08-07 (국내 카탈로그 — 추천 풀 36 → 51건 · 브랜드 12개)


## 2026-08-07 국내 카탈로그 — 추천 풀 51건 (배포 대기)

next_task: Klairs 227~230 이 쓰는 `설탕단풍수액`·`꽃단풍나무껍질추출물`·`리마콩씨추출물` 을
성분 사전에 적재 (식약처 적재가 slug 충돌로 0행에서 멈춘 건과 같은 뿌리를 먼저 진단)
...
코드는 `chore/post-deploy-docs-20260804` `6bd7ab0` 까지 푸시됨. **아직 main 아님.**
캐시 버전은 V2 → V3 로 올려 두었다 (풀이 36 → 51 로 늘어 옛 Top 5 가 낡았다).

### 아직 막힌 것 (지어내지 않고 남긴다)

- Klairs 227~230 — 위 3개 성분이 사전 1,341행에 **실제로 없다.** 이명이 아니라 사전 공백.
- Round Lab 몰 86건 — `availability` 를 안 주므로 재고를 추측하지 않는다.
